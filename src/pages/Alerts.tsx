...
const SEV_CONFIG: Record<string, { color: string; bar: string; label: string }> = {
  critical: { color: "bg-destructive text-destructive-foreground", bar: "bg-destructive", label: "Κρίσιμο" },
  high: { color: "bg-warning text-warning-foreground", bar: "bg-warning", label: "Υψηλό" },
  medium: { color: "bg-[hsl(50,80%,50%)] text-foreground", bar: "bg-[hsl(50,80%,50%)]", label: "Μεσαίο" },
  low: { color: "bg-accent text-accent-foreground", bar: "bg-accent", label: "Χαμηλό" },
};

const TYPE_LABELS: Record<string, string> = {
  price_increase: "Αύξηση τιμής",
  duplicate_invoice: "Διπλότυπο",
  missing_field: "Ελλιπές πεδίο",
  unusual_amount: "Ασυνήθιστο ποσό",
  overdue_payment: "Ληξιπρόθεσμη",
  budget_exceeded: "Υπέρβαση budget",
};
...
const Alerts = () => {
  ...
  const handleResolve = async (id: string) => {
    ...
    if (err) toast({ title: "Σφάλμα", description: err.message, variant: "destructive" });
    else toast({ title: "Επιτυχία", description: "Η ειδοποίηση επιλύθηκε" });
  };
      ...
      <div className="space-y-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-bold font-display text-foreground">Ειδοποιήσεις</h1>
          </div>
          <p className="text-muted-foreground">Ενημερώσεις και σημαντικές ειδοποιήσεις</p>
        </div>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <Select value={filterSeverity} onValueChange={setFilterSeverity}>
                <SelectTrigger className="w-36 h-8 text-xs"><SelectValue placeholder="Σοβαρότητα" /></SelectTrigger>
                <SelectContent>
              ...
              {types.length > 0 && (
                <Select value={filterType} onValueChange={setFilterType}>
                  <SelectTrigger className="w-40 h-8 text-xs"><SelectValue placeholder="Τύπος" /></SelectTrigger>
                  <SelectContent>
            ...
            {/* Alert cards */}
            {filtered.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground text-sm">Δεν βρέθηκαν ειδοποιήσεις</div>
            ) : (
                        ...
                        <CardContent className="flex-1 p-4 flex items-start justify-between gap-4">
                          <div className="space-y-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <Badge variant="outline" className="text-[10px]">{TYPE_LABELS[a.alert_type] ?? a.alert_type}</Badge>
                              <Badge className={cn("text-[10px]", sev.color)}>{sev.label}</Badge>
                              {a.is_resolved && <Badge variant="secondary" className="text-[10px]">Επιλύθηκε</Badge>}
                            </div>
                          ...
                          {!a.is_resolved && (
                            <Button size="sm" variant="outline" className="shrink-0 gap-1" onClick={() => handleResolve(a.id)}>
                              <CheckCircle className="w-3.5 h-3.5" /> Επίλυση
                            </Button>
                          )}
                        </CardContent>
                      </div>
