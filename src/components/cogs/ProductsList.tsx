...
export function ProductsList({ data, loading, categories, ingredients, costMap, onRefresh }: ProductsListProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({ title: t("toast.success"), description: t("cogs.success_product_delete") });
      onRefresh();
    } catch (err: unknown) {
      ...
      {loading ? (
        ...
        <div className="rounded-lg border bg-card divide-y">
          {filtered.map((item) => {
            ...
            const mDelivery = marginPercent(item.selling_price_delivery, cost);
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={item.type === "recipe" ? "default" : "secondary"} className="text-[10px] px-1.5 py-0">
                      {item.type === "recipe" ? t("cogs.product_type_recipe") : t("cogs.product_type_resale")}
                    </Badge>
                    {item.category && (
                  ...
                  <div className="text-xs text-muted-foreground flex items-center gap-3 mt-1">
                    <span className="font-mono">{t("cogs.col_cost")}: €{cost.toFixed(2)}</span>
                    ...
                    {item.selling_price_delivery > 0 && (
                      <span>
                        ...
                        <span className="font-mono">({mDelivery.toFixed(1)}%)</span>
                      </span>
                    )}
                  </div>
                </div>
